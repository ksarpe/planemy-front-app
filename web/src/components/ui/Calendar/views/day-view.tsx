...
export function DayView({ currentDate, events, onEventSelect, onEventCreate, onEventDelete }: DayViewProps) {
  ...
  return (
    <div
      data-slot="day-view"
      className="flex flex-col flex-1 rounded-xl bg-bg-secondary shadow-[-6px_-6px_16px_rgba(0,0,0,0.1)] overflow-hidden">
      {showAllDaySection && (
        <div className="border-b border-bg-primary bg-bg-primary">
          <div className="grid grid-cols-[3rem_1fr] sm:grid-cols-[4rem_1fr]">
            ...
            <div className="relative border-r border-bg-primary p-1 last:border-r-0 ">
      ...
      <div
        ...
        <div>
          {hours.map((hour, index) => (
            <div
              key={hour.toString()}
              className="relative h-[var(--week-cells-height)] border-b border-bg-primary last:border-b-0">
              {index > 0 && (
                <span className="absolute -top-3 left-0 flex h-6 w-16 max-w-full items-center justify-end pe-2 text-[10px] sm:pe-4 sm:text-xs bg-bg-secondary text-text-muted">
                  {format(hour, "h a")}
                </span>
              )}
            </div>
