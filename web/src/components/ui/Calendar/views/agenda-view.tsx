...
import { AgendaDaysToShow } from "@/components/ui/Calendar/constants";
import type { CalendarEvent } from "@/components/ui/Calendar/types";
import { getAgendaEventsForDay } from "@/components/ui/Calendar/utils";
...
export function AgendaView({ currentDate, events, onEventSelect }: AgendaViewProps) {
  ...
  return (
    <div className="flex-1 px-4 overflow-y-auto bg-bg-secondary rounded-xl shadow-[-6px_-6px_16px_rgba(0,0,0,0.1)]">
      {!hasEvents ? (
        <div className="flex min-h-[70svh] flex-col items-center justify-center py-16 text-center">
          <RiCalendarEventLine size={32} className="mb-2" style={{ color: "var(--color-text-muted)" }} />
