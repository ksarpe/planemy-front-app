...
import { EndHour, StartHour, WeekCellsHeight } from "@/components/ui/Calendar/constants";
import { type CalendarEvent } from "@/components/ui/Calendar/types";
import { isMultiDayEvent } from "@/components/ui/Calendar/utils";
import { DroppableCell } from "@/components/ui/Calendar/droppable-cell";
import { DraggableEvent } from "@/components/ui/Calendar/events/draggable-event";
import { EventItem } from "@/components/ui/Calendar/events/event-item";
import { useCurrentTimeIndicator } from "@/components/ui/Calendar/utils/use-current-time-indicator";
...
export function WeekView({ currentDate, events, onEventSelect, onEventCreate, onEventDelete }: WeekViewProps) {
  ...
  return (
    <div
      data-slot="week-view"
      className="flex h-full flex-col rounded-xl bg-bg-secondary shadow-[-6px_-6px_16px_rgba(0,0,0,0.1)] overflow-hidden">
      <div className="sticky top-0 z-30 grid grid-cols-[3.5rem_repeat(7,1fr)] border-b border-bg-primary backdrop-blur-md">
      ...
      {showAllDaySection && (
        <div className="border-b border-bg-primary bg-bg">
          <div className="grid grid-cols-[3.5rem_repeat(7,1fr)]">
            <div className="relative border-r border-bg-primary">
            ...
            {days.map((day, dayIndex) => {
              ...
              return (
                <div
                  key={day.toString()}
                  className="relative border-r border-bg-primary p-1 last:border-r-0"
                  data-today={isToday(day) || undefined}>
      ...
      <div ref={scrollContainerRef} className="grid flex-1 min-h-0 grid-cols-[3.5rem_repeat(7,1fr)] overflow-y-auto">
        {/* Time column */}
        <div className="border-bg-primary grid auto-cols-fr border-r bg-bg">
          {hours.map((hour, index) => (
            <div
              key={hour.toString()}
              className="border-bg-primary relative min-h-[var(--week-cells-height)]  flex items-center justify-center">
              {index > 0 && (
                <span className="absolute left-1/2 -translate-x-1/2 truncate -top-2 flex items-center justify-center text-[10px] sm:text-xs bg-bg-secondary text-text-muted">
                  {format(hour, "h a")}
                </span>
              )}
            </div>
        ...
        {days.map((day, dayIndex) => (
          <div
            key={day.toString()}
            className="border-bg-primary relative grid auto-cols-fr border-r last:border-r-0 bg-bg"
            data-today={isToday(day) || undefined}>
            {/* Positioned events */}
            ...
            {hours.map((hour) => {
              ...
              return (
                <div
                  key={hour.toString()}
                  className="border-bg-primary relative min-h-[var(--week-cells-height)] border-b last:border-b-0">
