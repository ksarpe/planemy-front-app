...
export default function TaskItem({ task }: TaskItemProps) {
  ...
  return (
    <li
      className={`border-l-4 rounded-lg p-4 transition-all duration-300 cursor-pointer transform
    ...
    >
      ...
      <div className="flex items-start gap-3 flex-1">
        ...
        <h3 className={`font-medium text-sm leading-5 transition-colors duration-200 ${
          task.isCompleted ? "line-through text-gray-500" : 
          clickedTask?.id === task.id ? "text-blue-800" : "text-gray-900"
        }`}>
          {task.title}
        </h3>
        ...
        {task.description && (
          ...
          clickedTask?.id === task.id ? "text-blue-600" : "text-gray-600"
          }`}>
          {task.description}
          </p>
        )}
        ...
        {/* Due date */}
        {task.dueDate && (
          <div className={`flex items-center gap-1 mt-2 text-xs transition-colors duration-200 ${
          isOverdue() ? 'text-red-600' :
          isDueSoon() ? 'text-yellow-600' :
          clickedTask?.id === task.id ? 'text-blue-600' : 'text-gray-500'
          }`}>
          <Calendar size={12} />
