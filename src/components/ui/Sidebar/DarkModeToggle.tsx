...
const TOGGLE_CLASSES = "text-sm font-medium flex items-center gap-2 px-3 py-2 transition-colors relative z-10 cursor-pointer";
...
export function DarkModeToggle({ collapsed = false }: DarkModeToggleProps) {
  ...
  return (
    <div className="relative flex w-fit items-center rounded-md bg-bg-alt overflow-hidden">
      <button
        className={`${TOGGLE_CLASSES} ${isDark ? "text-white" : "text-text-muted"}`}
        onClick={() => toggleDark()}>
