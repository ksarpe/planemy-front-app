...
export const PaymentDetailsPanel = ({ payment }: PaymentDetailsPanelProps) => {
  ...
  const handleUpdateReminderDays = async (newDays: string) => {
    ...
    if (isNaN(days) || days < 0) {
      throw new Error("Nieprawidłowa liczba dni");
    }
  ...
  return (
    <div className="px-3 md:px-4 pb-3 md:pb-4 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-slate-50">
      {/* Status and Description */}
      <div className="flex items-center justify-between py-3 border-b border-gray-100">
        <div className="flex items-center gap-2">
          <h4 className="font-medium text-gray-800">Szczegóły płatności</h4>
          {getStatusBadge()}
        </div>
      </div>
      ...
      <div className="py-3 border-b border-gray-100">
        <div className="flex items-start gap-2 text-sm text-gray-700">
          ...
          <div className="flex-1">
            <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Opis</p>
            <EditableText
              value={payment.description || "Dodaj opis..."}
              onSave={handleUpdateDescription}
              placeholder="Dodaj opis płatności..."
              className="w-full"
            />
          </div>
        </div>
      ...
      <div className="py-4">
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          {/* Nazwa płatności */}
          <div className="lg:col-span-2 flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100 shadow-sm">
            <div className="p-2 bg-indigo-100 rounded-lg">
              <Tag size={16} className="text-indigo-600" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Nazwa</p>
              <EditableText
                value={payment.name}
          ...
          <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Kwota</p>
              <EditableText
                value={payment.amount.toFixed(2)}
                onSave={handleUpdateAmount}
                type="number"
                suffix={`${payment.currency} / ${payment.cycle}`}
                className="w-full"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Przypomnienie</p>
              <EditableText
                value={payment.reminderDays.toString()}
                onSave={handleUpdateReminderDays}
                type="number"
                suffix="dni wcześniej"
                className="w-full"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 mt-1">
                {daysUntil >= 0 ? `Za ${daysUntil} dni` : `${Math.abs(daysUntil)} dni temu`}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100 shadow-sm">
            ...
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">Ostatnia</p>
              <p className="font-semibold text-gray-800 text-sm">
                {payment.lastPaymentDate ? format(new Date(payment.lastPaymentDate), "dd.MM.yyyy") : "Brak danych"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="py-3 border-t border-gray-100">
        <div className="flex flex-wrap gap-3 text-xs">
          <div className="flex items-center gap-1">
            ...
            <span className="text-gray-600">Auto-odnowienie: {payment.autoRenew ? "Włączone" : "Wyłączone"}</span>
          </div>
          <div className="flex items-center gap-1">
            <CreditCard size={12} className="text-gray-500" />
            <span className="text-gray-600">Kategoria: {payment.category}</span>
          </div>
          <div className="flex items-center gap-1">
            <TrendingUp size={12} className="text-gray-500" />
            <span className="text-gray-600">
              Roczny koszt: {(payment.amount * (payment.cycle === "monthly" ? 12 : 1)).toFixed(2)} {payment.currency}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-100">
        {!isPaidForCurrentPeriod && (
          <button
            ...
            <Check size={14} />
            <span className="hidden sm:inline">Oznacz jako opłacone</span>
            <span className="sm:hidden">Opłać</span>
          </button>
        )}
